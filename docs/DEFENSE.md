...
> embeddings and linear models, no fine-tuning, no deep network. The result is still
> useful as a triage aid, and every claim is backed by a leakage-safe,
...
> don't have that; we have noisy social video. What matters about this project is
...
the right direction from sound alone?", the measured answer is yes.
...
When the data is clean and the train/test distributions match, the crude method
...
sound classification (DCASE Task 2 in-the-wild: mid-70s to low-80s AUROC). We match
the field, not under-perform it, with a 100 KB linear model, and we
...
The ranking carries information all the way down, which is exactly what a
...
a calibrated fault/normal screen at the literature ceiling, a dependable "where in the
...
than chance. For a deliberately crude proof of concept, that is the result working
as designed, not a disappointment.
