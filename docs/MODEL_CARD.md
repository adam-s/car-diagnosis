...
possible. The case for why a deliberately crude method earns these numbers is in
...
- **Intended:** a calibrated triage aid. Flag fault vs normal, narrow where in
  ...
  demonstration of a scrape→clean→train→serve loop.
...
slice of the scraped social corpus (YouTube + TikTok; Reddit dropped as uncurated
...
classification (DCASE Task 2: mid-70s to low-80s AUROC). The same method reaches
**0.93 AUROC** trained+tested in-domain on clean engine audio, so the ceiling here
is the data, not the method.

**The knock head.** In-distribution it scores AUROC 0.99; on a
...
engine knock. It is therefore demoted, kept as a hint and never the headline, and
...
more domains; with ~4 it can't span the space.
