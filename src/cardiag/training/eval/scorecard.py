...
def run(out_md: Path | None = None) -> dict:
    ...
    report = {"corpus": {"n_clips": len(X), "n_videos": len(set(VID)),
                         "by_source": {k: int(v) for k, v in Counter(SRC).items()},
                         "by_kind": {k: int(v) for k, v in Counter(d["kind"]).items()}}}
    ...
    for name, m in masks.items():
        ...
        g = lambda k: f"{a[k][0]:.3f}±{a[k][1]:.2f}"
        binc = "/".join(f"{k}:{v}" for k, v in sorted(Counter(ys).items()))
        lines.append(f"| {name} | {len(ys)} | {binc} | {g('bal_acc')} | {g('macro_f1')} | "
                     f"{g('mcc')} | {g('auroc')} | {g('auprc')} | {ec:.3f} | {maj:.3f} |")
